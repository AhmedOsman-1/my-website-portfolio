"use client";

import { useState, useEffect } from "react";
import Image from "next/image";
import Link from "next/link";

const Navbar = () => {
  const [menuOpen, setMenuOpen] = useState(false);
  const [isScrolled, setIsScrolled] = useState(false);
  const [mounted, setMounted] = useState(false); // client-only

  useEffect(() => {
    setMounted(true);

    const handleScroll = () => setIsScrolled(window.scrollY > 20);
    window.addEventListener("scroll", handleScroll);

    return () => window.removeEventListener("scroll", handleScroll);
  }, []);

  if (!mounted) return null; // prevent server/client mismatch

  const scrollToSection = (id) => {
    const el = document.getElementById(id);
    if (el) el.scrollIntoView({ behavior: "smooth" });
    setMenuOpen(false);
  };

  const navItems = ["Projects", "About", "Services", "Contact"];

  return (
    <>
      {/* Background Shape */}
      <div className="fixed top-0 right-0 w-11/12 -z-10 translate-y-[-80%] pointer-events-none">
        <Image src="/bg-logo.jpeg" height={500} width={1000} alt="background logo" />
      </div>

      {/* Navbar */}
      <nav
        className={`w-full fixed px-5 lg:px-8 xl:px-[8%] flex items-center justify-between z-50 transition-all duration-300
        ${isScrolled ? "bg-white/60 backdrop-blur-md shadow-sm" : "bg-transparent"}`}
      >
        <Link href="/" className="flex items-center gap-2">
          <Image src="/logo.png" width={130} height={120} alt="logo" priority />
        </Link>

        <ul className="hidden mt-1 md:flex items-center gap-8 px-10 py-2 bg-white/60 backdrop-blur-md border border-gray-200 rounded-full shadow-sm">
          {navItems.map((item) => (
            <li key={item}>
              <button
                onClick={() => scrollToSection(item.toLowerCase())}
                className="font-Poppins relative group"
              >
                {item}
                <span className="absolute left-0 bottom-[-4px] w-0 h-[2px] bg-[#00a6ff] transition-all group-hover:w-full"></span>
              </button>
            </li>
          ))}
        </ul>

        <div className="flex items-center gap-4">
          <button className="p-2 rounded-full hover:bg-gray-100 transition">
            <Image className="w-6" src="/nightMode.png" width={20} height={20} alt="night mode" />
          </button>

          <button
            onClick={() => scrollToSection("contact")}
            className="hidden lg:flex items-center gap-3 btn-primary px-8 py-2.5"
          >
            Let&#39;s Talk
            <Image src="/arrowRight.png" width={20} height={20} alt="arrow" />
          </button>

          <button className="block md:hidden ml-3" onClick={() => setMenuOpen(true)}>
            <Image className="w-6" src="/menu2.png" width={24} height={24} alt="Menu" />
          </button>
        </div>
      </nav>

      {menuOpen && (
        <div className="fixed top-0 right-0 h-full w-full flex justify-center items-start z-[60] pointer-events-auto bg-black/10 backdrop-blur-sm">
          <div className="bg-white w-4/5 max-w-sm mt-16 rounded-xl shadow-xl flex flex-col items-center overflow-hidden">
            <div className="flex justify-end w-full px-4 py-4">
              <button onClick={() => setMenuOpen(false)}>
                <Image src="/close-black.png" width={28} height={28} alt="cancel" className="w-7" />
              </button>
            </div>

            <ul className="flex flex-col items-center w-full mt-2">
              {navItems.map((item) => (
                <li key={item} className="w-full border-b border-gray-200 text-center py-3">
                  <button
                    onClick={() => scrollToSection(item.toLowerCase())}
                    className="font-Poppins text-lg hover:text-[#00a6ff] transition"
                  >
                    {item}
                  </button>
                </li>
              ))}
            </ul>

            <div className="w-3/4 my-6 text-center">
              <button
                onClick={() => scrollToSection("contact")}
                className="btn-primary px-6 py-3 w-full text-lg rounded-full shadow-lg hover:shadow-xl transition-all duration-300"
              >
                Let&#39;s Talk
              </button>
            </div>
          </div>
        </div>
      )}
    </>
  );
};

export default Navbar;
